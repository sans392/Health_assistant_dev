...
- Уровень детализации зависит от данных в «Результаты анализа»:
  · если есть блок «По дням» — приведи в ответе разбивку по датам, не только сумму.
  · если есть блок «По тренировкам» — упомяни конкретные сессии (дата, тип, длительность).
  · если есть только агрегаты — отвечай суммарно, без построчных деталей и
    БЕЗ предложений «хочешь детализацию» — таких данных в payload нет."""
